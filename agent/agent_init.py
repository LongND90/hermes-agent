...
def init_agent(
    ...
    if api_mode in {"chat_completions", "codex_responses", "anthropic_messages", "bedrock_converse", "codex_app_server", "augment_rest"}:
        agent.api_mode = api_mode
    elif agent.provider == "augment-rest":
        agent.api_mode = "augment_rest"
    ...
    if (
        ...
        and agent.provider not in {"copilot-acp", "auggie-acp"}
        and not str(agent.base_url or "").lower().startswith("acp://copilot")
        and not str(agent.base_url or "").lower().startswith("acp://auggie")
    ...
    if agent.api_mode == "anthropic_messages":
    ...
    elif agent.api_mode == "augment_rest":
        ...
        agent.client = AugmentRestClient(
        ...
        if api_key and base_url:
            ...
            if agent.provider in {"copilot-acp", "auggie-acp"}:
